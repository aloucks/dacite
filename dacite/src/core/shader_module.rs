...
pub struct FromNativeShaderModuleParameters {
    /// `true`, if this `ShaderModule` should destroy the underlying Vulkan object, when it is dropped.
    pub owned: bool,

    /// The `Device`, from which this `ShaderModule` was created.
    pub device: Device,

    /// An `Allocator` compatible with the one used to create this `ShaderModule`.
    ///
    /// This parameter is ignored, if `owned` is `false`.
    pub allocator: Option<Box<core::Allocator>>,
}

impl FromNativeShaderModuleParameters {
    #[inline]
    pub fn new(owned: bool, device: Device, allocator: Option<Box<core::Allocator>>) -> Self {
        FromNativeShaderModuleParameters {
            owned: owned,
            device: device,
            allocator: allocator,
        }
...
impl FromNativeObject for ShaderModule {
    type Parameters = FromNativeShaderModuleParameters;

    unsafe fn from_native_object(object: Self::NativeVulkanObject, params: Self::Parameters) -> Self {
        ShaderModule::new(object, params.owned, params.device, params.allocator.map(AllocatorHelper::new))
    }
}

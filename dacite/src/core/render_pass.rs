...
pub struct FromNativeRenderPassParameters {
    /// `true`, if this `RenderPass` should destroy the underlying Vulkan object, when it is dropped.
    pub owned: bool,

    /// The `Device`, from which this `RenderPass` was created.
    pub device: Device,

    /// An `Allocator` compatible with the one used to create this `RenderPass`.
    ///
    /// This parameter is ignored, if `owned` is `false`.
    pub allocator: Option<Box<core::Allocator>>,
}

impl FromNativeRenderPassParameters {
    #[inline]
    pub fn new(owned: bool, device: Device, allocator: Option<Box<core::Allocator>>) -> Self {
        FromNativeRenderPassParameters {
            owned: owned,
            device: device,
            allocator: allocator,
        }
...
impl FromNativeObject for RenderPass {
    type Parameters = FromNativeRenderPassParameters;

    unsafe fn from_native_object(object: Self::NativeVulkanObject, params: Self::Parameters) -> Self {
        RenderPass::new(object, params.owned, params.device, params.allocator.map(AllocatorHelper::new))
    }
}
